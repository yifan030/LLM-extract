...
5. **参考答案提取**：如果文档末尾有「参考答案」「数学参考答案」等章节（通常以 `#` 标题分隔），直接从该章节提取答案原文填入 `answer` 字段，不要自行推断或求解：
   - **选择题**：从答案表格的「答案」行读取对应题号的选项字母（如 B、A、D）
   - **填空题**：从答案区的题号行提取答案文本，保留 LaTeX 公式原文
   - **解答题**：`answer` 留空（答案区为完整解题过程，不提取）
   - 如果文档没有参考答案章节，则客观题尽量给出 answer，解答题留空。
