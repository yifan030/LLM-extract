...
async def test_create_edge_success():
    ...
    created, dup = await repo.create_edge(e)
...
@pytest.mark.asyncio
async def test_create_edge_duplicated():
    mock_resp = _make_resp(status_code=400, text="already exists")
    mock_client = _make_client(mock_resp)
    repo = _make_repo(mock_client)
...
async def test_get_vertex_edges_uses_direction_and_label():
    ...
    url = mock_client.get.call_args.args[0]
    assert "vertex_id=%22v1%22" in url
