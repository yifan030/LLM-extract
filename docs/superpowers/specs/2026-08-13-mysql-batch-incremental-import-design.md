...
| 答案处理 | 不单独处理，靠 `import_paper` 的 LLM 抽取内嵌答案 | `prompts/exam_extract.md` 已要求从「参考答案」章节按题号抽取 `answer`，试卷与答案不分离 |
...
- **不处理「试卷/答案分家」的文件**：批量导入把桶内每个 `.md` 都当作试卷跑 `import_paper`（答案内嵌时 LLM 会一并抽出）。若桶里存在极少数独立答案文件（与试卷不同名、无内嵌答案），它们会被当作试卷错误抽取。当前「答案基本内嵌」，接受此限制；此类文件仍走既有 `POST /import/answers`（显式传 `paper_id`）手动处理。后续可加文件名启发式（如含「答案」字样则跳过）规避。
